...
public class PaymentBalanceRefund extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "余额支付退款接口测试", timeOut = 600000, dataProvider = "PaymentBalanceRefund")
	public void PaymentBalanceRefund(Map<String, String> datadriven) throws Exception {
		...
		/*String aaa=MyConfig.getTimestamp(datadriven.get("timestamp"));
		String oid_partner=MyConfig.getOidPartner(datadriven.get("oid_partner"));
		System.out.println("aaa:"+aaa);
		System.out.println("oid_partner:"+oid_partner);*/
		...
		try {
			...
			refund.setUser_id(MyConfig.getOidPartner(datadriven.get("paymentBalance.user_id")));
...
/*				if(!datadriven.get("refund.Update").equals("")){
				...
				//refund.setUser_id(MyConfig.getOidPartner(datadriven.get("user_id")));
				refund1.setUser_id(MyConfig.getOidPartner(datadriven.get("paymentBalance.user_id")));
			...
			funcUtil.writeRet2Excel(this, "PaymentBalanceRefund",ret_code,ret_msg+accp_txno+Check);
	...
	@DataProvider(name = "PaymentBalanceRefund")
	public Iterator<Object[]> data4PaymentBalanceRefund() throws IOException {
		return new ExcelProvider(this, "PaymentBalanceRefund");
	}
