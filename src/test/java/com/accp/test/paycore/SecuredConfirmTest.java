...
public class SecuredConfirmTest extends AbstractTestNGSpringContextTests {
	...
	@Test(description = "担保交易确认接口测试", timeOut = 600000, dataProvider = "securedConfirm")
	public void securedConfirm(Map<String, String> datadriven) throws Exception {
		...
		try {// 创单
			...
			// 担保交易确认
			if(!datadriven.get("securedConfirm.Update").equals("")){
				
				//Reporter.log("1111 :" + datadriven.get("securedConfirm.UpdateType"), true);
				switch (datadriven.get("securedConfirm.UpdateType")) {
				case "1":
					dataBaseAccessPayCore.updatePayConfiguration(datadriven.get("securedConfirm.Update")+"'"+accp_txno+"'");	
					break;
				case "2":
					dataBaseAccessPayCore.insertAcctConfiguration(datadriven.get("securedConfirm.Update")+"'"+MyConfig.getAccNO("get_mchassure_P")+"'");	
					break;
				case "3":
					dataBaseAccessPayCore.insertAcctConfiguration(datadriven.get("securedConfirm.Update")+"'"+MyConfig.getAccNO("get_payee_user_P")+"'");
					break;
				}
			}
			...
			if(!datadriven.get("securedConfirm1.private_key").equals("")){
				...
				securedConfirm1.setUser_id(MyConfig.getOidPartner(datadriven.get("securedConfirm1.user_id")));
				...
				confirmOrderInfo1.setConfirm_seqno(MyConfig.getConfirmSeqno(datadriven.get("securedConfirm1.confirm_seqno")));
				...
				Reporter.log("担保交易二次确认请求报文:" + reqJson4, true);
				...
				Reporter.log("担保交易二次确认返回报文:" + resJson4, true);
			}
			
			
			// 担保交易确认
			if(!datadriven.get("securedConfirm.Update1").equals("")){
				
				//Reporter.log("1111 :" + datadriven.get("securedConfirm.UpdateType"), true);
				switch (datadriven.get("securedConfirm.UpdateType")) {
				case "1":
					dataBaseAccessPayCore.updatePayConfiguration(datadriven.get("securedConfirm.Update1")+"'"+accp_txno+"'");	
					break;
				case "2":
					dataBaseAccessPayCore.insertAcctConfiguration(datadriven.get("securedConfirm.Update1")+"'"+MyConfig.getAccNO("get_mchassure_P")+"'");	
					break;
				case "3":
					dataBaseAccessPayCore.insertAcctConfiguration(datadriven.get("securedConfirm.Update1")+"'"+MyConfig.getAccNO("get_payee_user_P")+"'");
					break;
				}
			}
			 //验证金额
			...
			funcUtil.writeRet2Excel(this, "securedConfirm",ret_code, ret_msg+accp_txno+Check);
